...
EVAL = {
    ...
    ("beverage_rice_milk", "L.ar.rancid_oxidized"):
        "이 트랙에서는 신선 시료에서 매긴 점수입니다(review_items 참조). 훈련되지 않은 "
        "패널은 산화를 늦게, 들쭉날쭉하게 감지하므로 신뢰도가 낮습니다 — 작은 차이는 "
        "잡음으로 보세요. 이 축이 의사결정을 좌우한다면 블라인드 반복이 필요합니다"
        "(스펙 5.4).",
    ("beverage_rice_milk", "L.tx.body"):
        "패널 컬럼 이름은 'mouthfeel' 입니다. Layer L 이 SA.body_mouthfeel 을 "
        "L.tx.body 로 합쳤으므로(_deprecated_aliases.yaml) 판단이 아니라 그대로 이어진 "
        "대응입니다.",
